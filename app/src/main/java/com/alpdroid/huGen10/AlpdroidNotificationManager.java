...
public class AlpdroidNotificationManager {
  ...
  private static final int ALPDROID_ID = 1;
  private static final int AUTH_ERROR_ID = 2;

  private static final String CHANNEL_ID_ALPDROID = "alpdroid";
...
/*
  public AlpdroidNotificationManager(
    ...
    deliveryBroadcastReceiver=new NotificationDismissedReceiver();

            context.registerReceiver(
        deliveryBroadcastReceiver, new IntentFilter(NOTIFICATION_DISMISS_ACTION));
    ...
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O){
      ...
      NotificationChannel scrobbleChannel =
          new NotificationChannel(
              CHANNEL_ID_ALPDROID,
              context.getString(R.string.notification_channel_name_alpdroid),
  ...
  public void notifyScrobbled(Track track, int count) {
    if (!sharedPreferences.getBoolean("notifications_alpdroid", true)) {
      return;
    }
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      notificationBuilder.setChannelId(CHANNEL_ID_ALPDROID);
    }
    ...
    notificationManager.notify(ALPDROID_ID, notificationBuilder.build());
  }
